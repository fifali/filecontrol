...
namespace filecontrol
{
    public partial class Form1 : Form
    {
        ...
        //}
        public void init(string FtpServerIP, string FtpRemotePath, string FtpUserID, string FtpPassword)
        {
        ...
        //            Form1 fw = new Form1(ftpServerIP, folderToDelete, ftpUserID, ftpPassword);
        ...
        {
            init(this.ip.Text,this.dict.Text,this.user.Text,this.pass.Text);
        }
        ...
        {
            init(this.ip.Text, this.dict.Text, this.user.Text, this.pass.Text);
            Download(this.paths.Text, this.names.Text);
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                var ftp = new SFtp(this.ip2.Text, Convert.ToInt32(this.port2.Text), this.user2.Text, this.pass2.Text);
                if (!ftp.Connect())
                { return; };
                //ftp.Get("/home/admin/cjavapy.db", @"F:\Home\cjavapy.db");
                ftp.Get(this.fpath.Text, this.lpath.Text);
                ftp.Disconnect();
                MessageBox.Show("下载成功！");
            }
            catch (Exception ex)
            {
                Insert_Standard_ErrorLog.Insert("sFtpWeb", "Download Error --> " + ex.Message);
            }
        }
    }
